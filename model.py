# -*- coding: utf-8 -*-
"""
Created on Fri Nov  2 19:31:45 2018
...
class NTEP(object):
    def __init__(self, config, sess, node_embed):
        ...
        truth = tf.log(self.sz)
        # Define loss and optimizer
        
        cost = tf.reduce_mean(tf.pow(tf.log(self.pred) - truth, 2)) + self.scale*tf.add_n([self.regularizer(var) for var in tf.trainable_variables()])
        
        error = tf.reduce_mean(tf.pow(tf.log(self.pred) - truth, 2))
...
#        train_op = optim.apply_gradients(capped_gvs)
    ...
    def build_input(self):
        ...
        self.event=tf.placeholder(tf.float32,[None,50],name="event")
        ...
        #with tf.device('/gpu:1'):
            with tf.variable_scope('deepcas') as scope:
                ...
                with tf.variable_scope('BiGRU'):
                    ...
                    outputs, _, _ = tf.nn.static_bidirectional_rnn(self.gru_fw_cell, self.gru_bw_cell, x_vector,dtype=tf.float32)
                   # outputs, _, _ = rnn.bidirectional_rnn(self.gru_fw_cell, self.gru_bw_cell, x_vector,dtype=tf.float32)
                    print("2222222")
                ...
                with tf.variable_scope('attention'):
                    ...
                    print(attention_batch_seq)#(?,200)
                    #p_event=tf.reshape(self.event,[-1,1])
                    p_event1=tf.concat([self.event,self.event],1)
                    p_event2=tf.concat([self.event,self.event],1)
                    p_event=tf.concat([p_event1,p_event2],1)
                   # print(p_event)
                    #p_event=p_event
                    #p_event=tf.concat([self.event],1)
                    print(p_event)
                    print(self.event)
                    attention_batch_seq=attention_batch_seq
                    #attention_batch_seq=attention_batch_seq
                    attention_batch_lin = tf.reshape(attention_batch_seq, [-1, 1])
                    
                    
                    print(attention_batch_lin)#(?,1) (6400,1)
                    print(p_event)#(?,1) (1600,1)
    ...
    def train_batch(self, x, y, sz,event):
        self.sess.run(self.train_op, feed_dict={self.x: x, self.y: y, self.sz: sz,self.event:event})
        
    def get_error(self, x, y, sz,event):
        return self.sess.run(self.error, feed_dict={self.x: x, self.y: y, self.sz: sz,self.event:event})
    
    def get_pred(self,x,y,sz,event):
        return self.sess.run(self.pred, feed_dict={self.x: x, self.y: y, self.sz: sz,self.event:event})
